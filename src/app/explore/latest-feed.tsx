...
export function LatestFeed({ selectedTag }: LatestFeedProps) {
  ...
  const fetchLatestPosts = React.useCallback(
    async (isInitial: boolean) => {
      ...
      try {
        const data = await getPublicLatestPostsAction({
        ...
        if (isInitial) {
          ...
          // Filter duplicates to prevent layout bugs
          setPosts((prev) => {
  ...
  }, [hasMore, loadingMore, loading, fetchLatestPosts]);
  ...
  if (posts.length === 0) {
    return (
      <EmptyState
        ...
        description={selectedTag ? `There are no posts categorized with tag #${selectedTag} yet.` : "The community feed is currently empty."}
      />
    );
  }
